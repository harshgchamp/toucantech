...
class MY_Model extends CI_Model {
    ...
    function update_batch($table_name, $data_array , $where) {

        $this->db->update_batch($table_name, $data_array, $where);
    ...
    function get_list_order($table_name, $index_array, $columns = null,  $order_array = null, $limit = null) {
    ...
    function get_single($table_name, $index_array, $columns = null) {
        ...
        $row = $this->db->get_where($table_name, $index_array)->row_array();
...
// get data with paging
    ...
    public function get_custom_id_original($table, $prefix)
    {
      $max_id = '';
    ...
    public function get_custom_id($table, $column , $index_array , $prefix = '' , $suffix = '')
    {
      $max_id = '';
      ...
      $this->db->limit(1);
      $this->db->order_by("id", "desc");
      $arr = $this->db->get($table)->row_array();
    ...
    function get_only_column($table_name, $index_array, $column , $order_type = null) {
        if ($column){
            $this->db->select($column);
        }
        if ($order_type) {
   ...
   function duplicate_simple_id_check($tbl_name,$col_name,$name, $id = null) {
        if ($id) {
    ...
    function get_only_id($table_name, $index_array, $column) {
        if ($column){
            $this->db->select($column);
        }
    ...
    function get_array_coluumn($table_name, $index_array, $column) {
        $row = $data = array();
        if ($column){
            $this->db->select($column);
        }
        $row = $this->db->get_where($table_name, $index_array)->result_array();
    ...
    function get_operations_list($module_id){
        $this->db->select('b.operation_display_name,b.operation_slug,b.id,b.operation_name');
        $this->db->from('sws_ins_plans_details AS a');
        $this->db->join('sws_operations AS b', 'a.subfeatures_id = b.id   ', 'left');
        $this->db->where('a.status', 1);
        $this->db->where('a.mark', 1);
        $this->db->where('b.status', 1);
        $this->db->where('a.subfeatures_id !=', 0);
        $this->db->where('a.features_id', $module_id);
    ...
    function get_data_arr_list($table_name, $where_column, $columns, $ids_arr, $sort_type) {
        $this->db->select($columns);
        $this->db->from($table_name);
        $this->db->where_in($where_column, $ids_arr);
        $this->db->order_by($where_column, $sort_type);
        return $this->db->get()->result_array();
    }
}
